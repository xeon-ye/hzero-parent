...
public class JobExecuteServiceImpl implements JobExecuteService {
    ...
    @Override
    public void jobExecute(JobDataDTO jobDataDTO, Authentication authentication) {
            ...
            if (schedulerConfig.isUploadLog()) {
                ...
                try {
                    url = fileClient.uploadFile(jobDataDTO.getTenantId(), HZeroService.Scheduler.BUCKET_NAME, BootSchedulerConstant.LOG_DIR,
                            jobDataDTO.getJobCode() + suffix, null, log);
                } catch (Exception e) {
    ...
    public String stopJob(Long jobId) {
        ...
        if (CollectionUtils.isNotEmpty(list)) {
            for (Thread thread : list) {
                ...
                String result = stopThread(thread, iJobHandler);
                // 任务停止补偿方法
                iJobHandler.onStop();
                if (Objects.equals(result, BootSchedulerConstant.Response.FAILURE)) {
                    // 若该任务有多个线程在执行，有一个线程不可停止，任务都不可停止
    ...
    private String stopThread(Thread thread, IJobHandlerAllowStop iJobHandler) {
