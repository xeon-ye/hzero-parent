...
public class ImportRegistry {
     ...
     * 通用校验链缓存map,启动时初始化
     */
    private static final List<ValidatorHandler> COMMON_VALIDATOR_LIST = new ArrayList<>();
    /**
     * 自定义校验链缓存map,启动时初始化
     */
    ...
    public static List<ValidatorHandler> getCommonValidatorList() {
        return COMMON_VALIDATOR_LIST;
    }

    public static void addCommonValidator(ValidatorHandler validator) {
        if (!COMMON_VALIDATOR_LIST.contains(validator)) {
            COMMON_VALIDATOR_LIST.add(validator);
        }
    }
