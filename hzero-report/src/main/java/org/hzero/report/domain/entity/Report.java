...
public class Report extends AuditDomain {
    ...
    public boolean validateReportTemplateType() {
        return !StringUtils.endsWith(ReportTypeEnum.DOCUMENT.getValue(), reportTypeCode) || !StringUtils.isBlank(templateTypeCode);
    }
    ...
    @NotNull
    @JoinTable(name = "datasetJoin", target = Dataset.class, on = @JoinOn(joinField = Dataset.FIELD_DATASET_ID))
    @Encrypt
