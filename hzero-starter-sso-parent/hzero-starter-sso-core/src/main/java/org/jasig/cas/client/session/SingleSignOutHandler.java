...
public final class SingleSignOutHandler {
    ...
    private List<CasLoginProcessor> casLoginProcessors = new ArrayList<>();
    private List<CasLogoutProcessor> casLogoutProcessors = new ArrayList<>();
    ...
    public void setCasLoginProcessors(List<CasLoginProcessor> casLoginProcessors) {
        this.casLoginProcessors = casLoginProcessors;
    }

    public void setCasLogoutProcessors(List<CasLogoutProcessor> casLogoutProcessors) {
        this.casLogoutProcessors = casLogoutProcessors;
    }
    ...
    private void recordSession(final HttpServletRequest request) {
        ...
        for (CasLoginProcessor processor : casLoginProcessors) {
            try {
                processor.process(request, null, token);
            } catch (Exception e) {
                LOGGER.error("cas login processor error, processor is {}", processor.getClass().getSimpleName());
            }
        }
     ...
     */
    private void destroySession(final HttpServletRequest request, HttpServletResponse response) {
        ...
        if (CommonUtils.isNotBlank(ticket)) {
            request.setAttribute(CasUtils.ATTRIBUTE_CAS_TICKET, ticket);
            final HttpSession session = this.sessionMappingStorage.removeSessionByMappingId(ticket);
            if (session != null) {
                final String sessionID = session.getId();
                LOGGER.debug("Invalidating session [{}] for token [{}]", sessionID, ticket);

                this.logoutStrategy.logout(request);
            }

            for (CasLogoutProcessor processor : casLogoutProcessors) {
                try {
                    processor.process(request, null);
                } catch (Exception e) {
                    LOGGER.error("cas logout processor error, processor is {}", processor.getClass().getSimpleName());
                }
            }
        }
    }
    ...
    private static boolean isServlet30() {
        try {
            HttpServletRequest.class.getMethod("logout");
            return true;
