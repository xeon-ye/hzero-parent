...
public class EncryptionService implements IEncryptionService {
    ...
    /**
     * 解密 id:tableName:token:userId
     *
     * @param value JSON data
     * @return Decrypted byte array
     */
    @Override
    public String decrypt(String value, String tableName) {
        return decrypt(value, tableName, TokenUtils.getToken());
    }

    @Override
    public String decrypt(String value, String tableName, String accessToken) {
    ...
    public Object decrypt(final JsonParser parser, final JsonDeserializer<?> deserializer, final DeserializationContext context, final JavaType javaType, Encrypt encrypt) {
        try {
            if (javaType.isCollectionLikeType() || javaType.isArrayType()) {
                ...
                for (JsonNode jsonNode : node) {
                    if (EncryptUtils.ignoreValue(encrypt, jsonNode.textValue())) {
                        ...
                        objectList.add(decrypt(jsonNode.textValue(), encrypt.value()));
                    } else {
            ...
            if (!EncryptUtils.ignoreValue(encrypt, decrypt)) {
                decrypt = decrypt(parser.getValueAsString(), encrypt.value());
            }
